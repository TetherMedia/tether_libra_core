...
//  glachac@tethermedia.com:
//
//  This is an altered copy of the SHA3Digest.dart file from PointyCastle

//  This file is patched to implement the non-keccak version of SHA3
//
//  See the explanation in the PointyCastle issues:
//   https://github.com/PointyCastle/pointycastle/issues/128
//
//  We presume at some point, PointCastle will roll this patch into
//  a released version, and we can switch to that.
//
...
class SHA3FixDigest extends BaseDigest implements Digest {
  static final RegExp _NAME_REGEX = RegExp(r"^SHA-3\/([0-9]+)$");

  /// Intended for internal use.
  static final FactoryConfig FACTORY_CONFIG = DynamicFactoryConfig(
      Digest,
      _NAME_REGEX,
      (_, final Match match) => () {
            int bitLength = int.parse(match.group(1));
            return SHA3FixDigest(bitLength);
          });

...
class Mnemonic {
  List<String> _mnemonic = List<String>();
  ...
  Mnemonic.fromPhrase(String phrase) {
    ...
    if (words.length < 6 || (words.length % 6 != 0)) {
      throw Exception("Mnemonic must have a word count divisible with 6");
    }
    var mnemonic = List<String>();
    for (String word in words) {
      int idx = binarySearch<String>(_WORDS, word);
      if ( idx > -1) {
        mnemonic.add(_WORDS[idx]);
      } else {
        throw Exception("Mnemonic contains an unknown word");
      }
    }
  ...
  Mnemonic.mnemonic(Uint8List data){
    if (data.length % 4 != 0) {
      throw Exception("Data for mnemonic should have a length divisible by 4");
    }

    var check = sha256.convert(data).bytes;

    var size = (data.length *8 + data.length/4);
    List<int> bits = List<int>(size.toInt());

    for (int i=0;i<data.length;i++){
      for (int j=0;j<8;j++){
        var value = (data[i] & (1 << (7 - j))) > 0;
        bits[i*8+j] = value?1:0;
      }

    }
    for (int  i=0;i<data.length/4;i++){
      var idx = i/8;
      var value = (check[idx.toInt()] & (1 << (7 - (i % 8)))) > 0;
      bits[8 * data.length + i] =  value?1:0;
    }
    var mlen = data.length * 3 / 4;
    var memo = List<String>();

    for (int i=0; i<mlen; i++){
      var idx = 0;
      for (int j=0;j<11;j++){
        var value = bits[i * 11 + j];
        if (value > 0) {
          idx += 1 << (10 - j);
        }
      }
